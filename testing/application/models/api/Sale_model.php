...
public function put_cart_save($sales,$pid,$uid)
  {    
  $this->db->update('cart_product', $sales, array('pid' => $pid,'uid' => $uid));
  $result = $this->db->affected_rows();
  return $result;
}

public function delete_cart_product($cpid)
    {
	$this->db->where(array('cpid' => $cpid));
	$this->db->delete('cart_product');

	return $this->db->affected_rows();
}

public function get_today_sale_amount($company_id)
  {

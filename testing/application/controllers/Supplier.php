...
public function save_supplier()
    ...
    $custMob = $this->db->select('mobile')->from('suppliers')->where('mobile',$info['mobile'])->get()->row();
  ...
  if($custMob)
    ...
    $log = [
        'activity' => 'Supplier is added ('.$cusid.')',
        'compid'       => $_SESSION['compid'],
...
public function update_supplier()
    ...
    $prname = $this->db->select('sup_id')
                        ->from('suppliers')
                        ->where('supplierID', $info['sup_id'])
                        ->get()
                        ->row();
  
  $log = [
      'activity' => 'Supplier is updated ('.$prname->sup_id.')',
      'user_id'       => $_SESSION['uid'],
      'compid'       => $_SESSION['compid'],
                ];
    $this->pm->insert_data('activity_logs',$log);
...
public function delete_supplier($id)
    ...
    if ($purchase)
        ...
        if($result)
            {
            $log = [
                'activity' => 'Supplier is deleted ('.$prname->sup_id.')',
                'user_id'       => $_SESSION['uid'],
                'compid'       => $_SESSION['compid'],
                            ];
                $this->pm->insert_data('activity_logs',$log);
...
public function view_supplier($id)
  ...
  $where = array(
    ...
    $field = array(
        'suppliers'  => 'suppliers.*',
        'purchase'  => 'purchase.purchaseDate,purchase.purchaseID,purchase.challanNo,purchase.totalPrice,purchase.paidAmount,purchase.due'
        );

    $join = array(
        'purchase' => 'purchase.supplier = suppliers.supplierID'
        
             );
             

  $supplier = $this->pm->get_data('suppliers',$where);
  $data['supplier'] = $supplier[0];
  
  $data['supp'] = $this->pm->get_data('suppliers',$where,$field,$join,$other);

  $this->load->view('suppliers/supplierView',$data);
}
